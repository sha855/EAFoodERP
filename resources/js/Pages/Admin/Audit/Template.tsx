...
export default function AuditTemplate({
  ...
  const auditTemplateShow = (id: any) => {
    router.get(
      route('admin.template.show', { template: id, company: companyId })
    );
  };

  const auditTemplateEdit = (id: any) => {
    router.get(
      route('admin.template.edit', { template: id, company: companyId })
    );
  };
  ...
  const handleConfirmDelete = () => {
    if (deleteId !== null) {
      router.delete(route('admin.template.destroy', { id: deleteId }), {
        onSuccess: () => {
          setNotification({
        ...
        <div className="flex-1 p-4 bg-gray-50">
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex gap-2">
              <CommonButton
                href={route('admin.template.create', { company: companyId })}
                variant="success"
              >
                New Audit Template
              </CommonButton>
          ...
          <Table
            columns={columns}
            datas={template}
            route={route('admin.template.index', { company: companyId })}
          />
        </div>
      </div>
    </AuthenticatedLayout>
