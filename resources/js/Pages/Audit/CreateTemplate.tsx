  ...
  const { data, setData, post, processing, errors } = useForm<
    {
      ...
      score_level: string;
      start_date: string;
      question: { id: number; text: string; language: string }[];
    } & { errors?: ErrorsType }
  >({
    name: '',
    audit_frequency: '',
            ...
            <div className="px-8 py-6 space-y-3">
              ...
              <div className="max-h-64 overflow-y-auto ">
                {data.question.map((q, index) => {
                  const questionError = (errors as any)[
                    `question.${index}.text` as keyof ErrorsType
                  ];
                  return (
                    <div key={q.id} className="mt-2 flex flex-col space-y-2">
                      ...
                      {(errors as any)[`question.${index}.text`] && (
                        <p className="text-red-500 text-sm mt-1">
                          {(errors as any)[`question.${index}.text`].replace(
                            /question\.\d+\./,
                            ''
                          )}
                        </p>
