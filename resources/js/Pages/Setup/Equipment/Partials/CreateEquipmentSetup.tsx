...
interface CreateEquipmentSetupProps {
  types: string[];
  rooms: any;
  edit: any;
...
export default function CreateEquipmentSetup({
  types,
  rooms,
  edit,
  setEdit,
  routePrefix,
}: CreateEquipmentSetupProps) {
  ...
  const filterRoom = useCallback(
    ({ id }: { id: number }): OptionType | undefined => {
      if (!rooms) return;

      const selectedData: any = Object.values(rooms).find(
        (item: any) => item.id === id
      );
  ...
  const selectOptionsRooms = useMemo(() => {
    return Object.values(rooms)?.map((item: any) => ({
      label: item?.name,
      value: item?.id,
    }));
  }, [rooms]);
  ...
  const handleSubmit = (e: any) => {
    e.preventDefault();
    post(route(routePrefix + 'equipment.store', adminRouteParams), {
      onSuccess: () => {
        dispatch(closeDrawer());
      },
    });
  ...
  const handleEditSubmit = (e: any) => {
    ...
    put(route(routePrefix + 'equipment.update', adminRouteParams), {
      onSuccess: () => {
        setEdit(null);
        dispatch(closeDrawer());
      },
  ...
  return (
    <form>
      ...
      <div className="mb-3">
        <InputLabel
          htmlFor="room"
          value={translations?.createModal?.form?.room}
          className="text-base mb-2"
        />
        <CommonSelectSearch
          options={selectOptionsRooms}
          onChange={(value) => {
            setData('room_id', (value as any)?.value);
          }}
          defaultValue={filterRoom({ id: edit?.room_id }) as OptionType}
        />
        {errors.room_id && (
          <p className="text-red-500 text-sm">{errors.room_id}</p>
        )}
      </div>

      <h3 className="text-xl font-bold mb-2">
        {translations?.createModal?.alert?.food_safety}
      </h3>
      {openAlert && (
